...
- Before initiating the Pixela upload, you can enter a comment for the day when Uploading to Pixela. The comment is created only for the 'Learning Tracker' main graph. It lets you capture e.g. a rating for the day or number of pages you read. Together with the comment, a timestamp is stored in the pixel optional data using the format as in the following example: 21:15 - 22:47. The Start time is the time you hit the start button the first time you open the GUI and the End time is the time you hit the Upload button. You can record timestamps and comments for several sessions within one day for one pixel, every new GUI session appends a new timestamp and comment to the existing pixel optional data upon hitting the Upload button. Please note that if you hit the Upload button multiple times in the same GUI session (i.e. when the start time is the same), the timestamp and comment gets replaced with the latest Upload data (timestamp + comment). This let's you track the times of day and comments of your learnings, e.g. when you learn in the morning and again in the evening.
